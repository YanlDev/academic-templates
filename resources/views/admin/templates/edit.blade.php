<x-admin-layout :breadcrumbs="[
    ...
    [
        'name' => 'Editar: ' . $template->name
    ]
]">

    <form action="{{ route('admin.templates.update', $template) }}" method="POST" enctype="multipart/form-data" id="template-form">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="mb-4 p-4 bg-gray-50 rounded-lg border">
                        ...
                        <div class="flex items-center mt-2">
                            ...
                            <div>
                                <p class="text-sm text-gray-900">{{ basename($template->excel_file) }}</p>
                                <p class="text-xs text-gray-500">Subido: {{ $template->created_at->format('d/m/Y') }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <label for="excel_file" class="cursor-pointer">
                            <div id="excel-upload-area">
                                ...
                                <p class="mt-2 text-sm text-gray-600">
                                    <span class="font-medium text-blue-600 hover:text-blue-500">Haz clic para cambiar archivo</span>
                                    o arrastra el nuevo archivo
                                </p>
                    ...
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-gray-500 text-xs mt-1">Dejar vacío para mantener el archivo actual</p>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        <div class="w-full h-64 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-500 transition-colors flex items-center justify-center overflow-hidden">
                            @if($template->main_image)
                                <img id="main-image-preview"
                                     src="{{ Storage::url($template->main_image) }}"
                        ...
                        <label for="main_image" class="absolute top-3 right-3 px-3 py-2 bg-white rounded-md shadow-sm hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer transition-all">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm text-gray-600">Cambiar</span>
                            </div>
                        </label>
                    ...
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-gray-500 text-xs mt-1">Dejar vacío para mantener la imagen actual</p>
                </div>
            </div>

            <!-- Imágenes de Preview Actuales -->
            @if($template->preview_images && count($template->preview_images) > 0)
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4">
                        @foreach($template->preview_images as $index => $previewImage)
            ...
            <div class="mb-4">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
                    ...
                    <label for="preview_images" class="cursor-pointer">
                        ...
                        <p class="mt-2 text-sm text-gray-600">
                            <span class="font-medium text-blue-600 hover:text-blue-500">Haz clic para subir nuevas imágenes</span>
                            o arrastra los archivos
                        </p>
                        <p class="text-xs text-gray-500">PNG, JPG (máx. 2MB cada una) - Reemplazará las actuales</p>
                    </label>
                </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-6">
            ...
            <div class="mb-4">
                ...
                <textarea name="features_text"
                          ...
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          required
                          oninput="countFeatures(this)">{{ old('features_text', implode("\n", $template->features ?? [])) }}</textarea>
                @error('features')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                    ...
                    reader.onload = function(e) {
                        ...
                        imageDiv.innerHTML = `
                            <img src="${e.target.result}"
                                 alt="Nuevo Preview ${index + 1}"
                                 class="w-full h-24 object-cover rounded-lg border-2 border-blue-300">
                            <div class="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                <span class="text-white text-xs">Nuevo ${index + 1}</span>
                            </div>
                        `;
